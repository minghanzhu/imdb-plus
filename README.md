# OpenCSD Content-Services
**Client**

`POST /client/`
* Description: This endpoint receives the user data for a new user to be created and retuns the user data plus the DynamoDB auto-generated clientId and accessToken.
* Sample request: `http://localhost:8083/client`
* Sample request body:
```
    {
        "clientname": "Netflix",
        "email": "services@netflix.com",
        "accountSetting": {
            "isPrivate": true,
            "isAdult": false
        }
...
```
 {
      "clientId": "18066abf-a30c-4dca-9cb9-845f6fa7d910",
      "accessToken": "2b7ae433-fc09-4035-aa3d-8fc37b676ae3",
      "clientname": "Netflix",
      "email": "services@netflix.com",
      "accountSetting": {
          "isPrivate": true,
...
* Header Authorization: `2b7ae433-fc09-4035-aa3d-8fc37b676ae3`
* Sample response:
```
    {
      "clientId": "18066abf-a30c-4dca-9cb9-845f6fa7d910",
...
`PUT /client/{id}`
* Description: This endpoint updates the user data for an existing user with `clientId` equal to `id`.
* Sample request: `http://localhost:8083/client/18066abf-a30c-4dca-9cb9-845f6fa7d910`
* Header Authorization: `2b7ae433-fc09-4035-aa3d-8fc37b676ae3`
* Sample request body:
```
    {
        "clientname": "Netflix",
        "email": "content_services@netflix.com",
        "accountSetting": {
            "isPrivate": true,
            "isAdult": false
        }
...
`DELETE /client/{id}`
* Description: This endpoint deletes the user data for an existing user with `clientId` equal to `id`. (Requied a header that has `"Authorization"` as key and user `accessToken` as value).
* Sample request: `http://localhost:8083/client/0d3cee16-3cb7-407a-a738-0212686c84d8`
* Sample response:
...
`GET /timeline/user/{clientId}`
* Description: This endpoint retrieves the timeline for the user whose `clientId` matches the given `clientId`.
...
[
    {
        ...
        "clientId": "c0cba403-b9c8-4181-9e7c-be9ab80a67c3",
        "mediaId": "tt0000019",
...
[
    {
        ...
        "clientId": "b4bd0ba7-3b8e-4634-ae69-9e487909eab1",
        "mediaId": "tt0000011",
    ...
    },
    {
        ...
        "clientId": "fc3bb87d-34be-4053-820f-df581b468b1e",
        "mediaId": "tt0000011",
...
* Description: This endpoint retrieves the timeline for the media whose `mediaId` matches the given `mediaId` as well as the user whose `clientId` matched `clientId`.
...
* Description: This endpoint deletes the timeline data for an existing timeline with user with specified `clientId` and `mediaId`. (Requied a header that has `"Authorization"` as key and user `accessToken` as value)
* Sample request:
...
* Description: This endpoint returns  a map of media and the user with clientId `id`'s genre preference and number of media with that.
...
* User can find Timeline by mediaId matches the given mediaId as well as the user whose clientId matched clientId:
